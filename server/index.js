...
function getTodoItem(itemId) {
  ...
  if (!todoItem) throw new TodoApiError(404, { message: 'Item Not Found' });
  return todoItem;
}
...
function updateTodoItem(itemId, data) {
  ...
  if (itemIndex === -1) throw new TodoApiError(404, { message: 'Item Not Found' });
    ...
    const body = await (async () => {
      ...
      if (uri === '' || uri === '/') {
        ...
        if (req.method === 'POST') {
          ...
          if (Math.random() > .9)
            throw new TodoApiError(404, { message: 'My error 1' });
          else
            res.statusCode = 201;
        ...
        if (req.method === 'PATCH') {
          if (Math.random() > .9)
            throw new TodoApiError(405, { message: 'My error 2' });
